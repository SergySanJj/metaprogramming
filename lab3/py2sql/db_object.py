...
class Column:
    def __init__(self, col_type: type, foreign_key=None, primary_key=False):
...
def foreign_keys(cls) -> List[Any]:
    ...
    for c in columns:
        ...
        if column.foreign_key:
            foreign.append(column.foreign_key)
    return foreign
...
def create_table_query(cls):
    ...
    for c in columns:
        ...
        if column.foreign_key:
            v = ""
            v += f"""\nFOREIGN KEY ({c}) REFERENCES {column.foreign_key.ref_table.__table_name__} ({column.foreign_key.ref_column})"""
